...
#            (3*3*self.ksize2+self.ntriag, 3*self.ksizep0p2 +self.ksizemu0+self.ntriag), 'float64')
        self.cov1 = np.zeros((4*self.ksize1+self.ntriag, 4*self.ksize1 +self.ntriag), 'float64')
        self.cov = np.zeros((3*self.ksizep0p2+self.ksizemu0+self.ntriag, 3*self.ksizep0p2+self.ksizemu0 +self.ntriag), 'float64')

        datafile = open(os.path.join(self.data_directory, self.covmat_file), 'r')
        for i in range(4*self.ksize1+self.ntriag):
            line = datafile.readline()
            while line.find('#') != -1:
                line = datafile.readline()
            for j in range(4*self.ksize1+self.ntriag):
...
#### standard deviations ######
        ...
        x1 = np.zeros(3*self.ksizep0p2 + self.ksizemu0+self.ntriag)
        ...
        EbG3cov = np.zeros(3*self.ksizep0p2+self.ntriag+ self.ksizemu0)
        Pshotcov = np.zeros(3*self.ksizep0p2+self.ntriag+ self.ksizemu0)
	Bshotcov = np.zeros(3*self.ksizep0p2+self.ntriag+ self.ksizemu0)
        c1cov = np.zeros(3*self.ksizep0p2+self.ntriag+ self.ksizemu0)

        a0cov = np.zeros(3*self.ksizep0p2+self.ntriag+ self.ksizemu0)
        a2cov = np.zeros(3*self.ksizep0p2+self.ntriag+ self.ksizemu0)
        Ecs4cov = np.zeros(3*self.ksizep0p2+self.ntriag+ self.ksizemu0)
        Ecs2cov = np.zeros(3*self.ksizep0p2+self.ntriag+ self.ksizemu0)
        Ecs0cov = np.zeros(3*self.ksizep0p2+self.ntriag+ self.ksizemu0)

        Eb4cov = np.zeros(3*self.ksizep0p2+self.ksizemu0+self.ntriag)
