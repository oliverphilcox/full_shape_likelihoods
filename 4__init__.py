...
#### standard deviations ######
	...
	Bshotcov = np.zeros(3*self.ksize2+self.ntriag)
        c1cov = np.zeros(3*self.ksize2+self.ntriag)
...
### bispectrum part starts
	...
	EBBshot = np.zeros(self.ntriag);
        ...
        for j in range(int(self.ntriag)):
                ...
                F2func1C = lambda k1,k2,k3,mu, phi:  z21(k1,k2,k3,mu,phi)*(1+beta*nu1(mu)**2)*(1. + beta*(nu2(k1, k2, k3, mu, phi))**2.)*PresC((kc1+k1*dk1/2.)*q1(mu),nu1(mu))*(kc1+k1*dk1/2.)*(dk1/2.)*PresC((kc2+k2*dk2/2.)*q2(k1, k2, k3, mu, phi),nu2(k1,k2,k3,mu,phi))*(kc2+k2*dk2/2.)*(dk2/2.)*(kc3+k3*dk3/2.)*(dk3/2.)+1.*0.5*(Bshot/ng)*b1**2.*PresC((kc1+k1*dk1/2.)*q1(mu),nu1(mu))*(kc1+k1*dk1/2.)*(1.+beta*nu1(mu)**2.*(Bshot+2.*(1.+Pshot))/Bshot + beta**2.*nu1(mu)**4.*2.*(1.+Pshot)/Bshot)*(kc2+k2*dk2/2.)*(kc3+k3*dk3/2.)*(dk1/2.)*(dk2/2.)*(dk3/2.) + ((1.+Pshot)/ng)**2.*(kc1+k1*dk1/2.)*(kc2+k2*dk2/2.)*(kc3+k3*dk3/2.)*(dk1/2.)*(dk2/2.)*(dk3/2.)/2.
                F2func2C = lambda k1,k2,k3,mu, phi:  z22(k1,k2,k3,mu,phi)*(1+beta*nu1(mu)**2)*(1. + beta*(nu3(k1, k2, k3, mu, phi))**2.)*PresC((kc1+k1*dk1/2.)*q1(mu),nu1(mu))*(kc1+k1*dk1/2.)*(dk1/2.)*PresC((kc3+k3*dk3/2.)*q3(k1, k2, k3, mu, phi),nu3(k1,k2,k3,mu,phi))*(kc3+k3*dk3/2.)*(dk3/2.)*(kc2+k2*dk2/2.)*(dk2/2.)+1.*0.5*(Bshot/ng)*b1**2.*PresC((kc2+k2*dk2/2.)*q2(k1, k2, k3, mu, phi),nu2(k1,k2,k3,mu,phi))*(kc2+k2*dk2/2.)*((1.+beta*nu2(k1,k2,k3,mu,phi)**2.*(Bshot+2.*(1.+Pshot))/Bshot + beta**2.*nu2(k1,k2,k3,mu,phi)**4.*2.*(1.+Pshot)/Bshot))*(kc1+k1*dk1/2.)*(kc3+k3*dk3/2.)*(dk1/2.)*(dk2/2.)*(dk3/2.) + 0.*(1/ng)**2.*(kc1+k1*dk1/2.)*(kc2+k2*dk2/2.)*(kc3+k3*dk3/2.)*(dk1/2.)*(dk2/2.)*(dk3/2.)/6.
                F2func3C = lambda k1,k2,k3,mu, phi:  z23(k1,k2,k3,mu,phi)*(1+beta*(nu2(k1, k2, k3, mu, phi))**2)*(1. + beta*(nu3(k1, k2, k3, mu, phi))**2.)*PresC((kc2+k2*dk2/2.)*q2(k1, k2, k3, mu, phi),nu2(k1,k2,k3,mu,phi))*(kc2+k2*dk2/2.)*(dk2/2.)*PresC((kc3+k3*dk3/2.)*q3(k1, k2, k3, mu, phi),nu3(k1,k2,k3,mu,phi))*(kc3+k3*dk3/2.)*(dk3/2.)*(kc1+k1*dk1/2.)*(dk1/2.) + 1.*0.5*(Bshot/ng)*b1**2.*PresC((kc3+k3*dk3/2.)*q3(k1, k2, k3, mu, phi),nu3(k1,k2,k3,mu,phi))*(kc3+k3*dk3/2.)*(1.+beta*nu3(k1,k2,k3,mu,phi)**2.*(Bshot + 2.*(1.+Pshot))/Bshot + beta**2.*nu3(k1,k2,k3,mu,phi)**4.*2.*(1.+Pshot)/Bshot)*(kc2+k2*dk2/2.)*(kc1+k1*dk1/2.)*(dk1/2.)*(dk2/2.)*(dk3/2.) + 0.*(1/ng)**2.*(kc1+k1*dk1/2.)*(kc2+k2*dk2/2.)*(kc3+k3*dk3/2.)*(dk1/2.)*(dk2/2.)*(dk3/2.)/6.
                ...
                Bfunc4 = lambda k1,k2,k3,mu, phi: ((2.*F2func1C(k1,k2,k3,mu, phi) + 2.*F2func2C(k1,k2,k3,mu, phi) + 2.*F2func3C(k1,k2,k3,mu, phi))/apar**2./aperp**4.) - ((2.*F2func1(k1,k2,k3,mu, phi) + 2.*F2func2(k1,k2,k3,mu, phi) + 2.*F2func3(k1,k2,k3,mu, phi))/apar**2./aperp**4.)


		#EPfunc = lambda k1,k2,k3,mu, phi:  (Bshot/ng)*b1**2.*Pres((kc1+k1*dk1/2.)*q1(mu),nu1(mu))*(kc1+k1*dk1/2.)*(1.+beta*nu1(mu)**2.*(Bshot+2.*(1.+Pshot))/Bshot + beta**2.*nu1(mu)**4.*2.*(1.+Pshot)/Bshot)*(kc2+k2*dk2/2.)*(kc3+k3*dk3/2.)*(dk1/2.)*(dk2/2.)*(dk3/2.) + ((1.+Pshot)/ng)**2.*(kc1+k1*dk1/2.)*(kc2+k2*dk2/2.)*(kc3+k3*dk3/2.)*(dk1/2.)*(dk2/2.)*(dk3/2.) + +(Bshot/ng)*b1**2.*Pres((kc2+k2*dk2/2.)*q2(k1, k2, k3, mu, phi),nu2(k1,k2,k3,mu,phi))*(kc2+k2*dk2/2.)*((1.+beta*nu2(k1,k2,k3,mu,phi)**2.*(Bshot+2.*(1.+Pshot))/Bshot + beta**2.*nu2(k1,k2,k3,mu,phi)**4.*2.*(1.+Pshot)/Bshot))*(kc1+k1*dk1/2.)*(kc3+k3*dk3/2.)*(dk1/2.)*(dk2/2.)*(dk3/2.) +(Bshot/ng)*b1**2.*Pres((kc3+k3*dk3/2.)*q3(k1, k2, k3, mu, phi),nu3(k1,k2,k3,mu,phi))*(kc3+k3*dk3/2.)*(1.+beta*nu3(k1,k2,k3,mu,phi)**2.*(Bshot + 2.*(1.+Pshot))/Bshot + beta**2.*nu3(k1,k2,k3,mu,phi)**4.*2.*(1.+Pshot)/Bshot)*(kc2+k2*dk2/2.)*(kc1+k1*dk1/2.)*(dk1/2.)*(dk2/2.)*(dk3/2.)

#		EBfunc = lambda k1,k2,k3,mu, phi:  (1./ng)*b1**2.*Pres((kc1+k1*dk1/2.)*q1(mu),nu1(mu))*(kc1+k1*dk1/2.)*(1.+beta*nu1(mu)**2.*3. + beta**2.*nu1(mu)**4.*2.)*(kc2+k2*dk2/2.)*(kc3+k3*dk3/2.)*(dk1/2.)*(dk2/2.)*(dk3/2.) + (1./ng)*b1**2.*Pres((kc2+k2*dk2/2.)*q2(k1, k2, k3, mu, phi),nu2(k1,k2,k3,mu,phi))*(kc2+k2*dk2/2.)*((1.+beta*nu2(k1,k2,k3,mu,phi)**2.*3. + beta**2.*nu2(k1,k2,k3,mu,phi)**4.*2.))*(kc1+k1*dk1/2.)*(kc3+k3*dk3/2.)*(dk1/2.)*(dk2/2.)*(dk3/2.) +(1./ng)*b1**2.*Pres((kc3+k3*dk3/2.)*q3(k1, k2, k3, mu, phi),nu3(k1,k2,k3,mu,phi))*(kc3+k3*dk3/2.)*(1.+beta*nu3(k1,k2,k3,mu,phi)**2.*3. + beta**2.*nu3(k1,k2,k3,mu,phi)**4.*2.)*(kc2+k2*dk2/2.)*(kc1+k1*dk1/2.)*(dk1/2.)*(dk2/2.)*(dk3/2.)
#		EPfunc = lambda k1,k2,k3,mu, phi: (1./ng)**0.*(kc1+k1*dk1/2.)*(kc2+k2*dk2/2.)*(kc3+k3*dk3/2.)*(dk1/2.)*(dk2/2.)*(dk3/2.)

		EBfunc = lambda k1,k2,k3,mu, phi: b1**2.*(((1.+beta*nu1(mu)**2.)*Pres((kc1+k1*dk1/2.)*q1(mu),nu1(mu))+Pres((kc2+k2*dk2/2.)*q2(k1, k2, k3, mu, phi),nu2(k1,k2,k3,mu,phi))*(1.+beta*nu2(k1,k2,k3,mu,phi)**2.)+ Pres((kc3+k3*dk3/2.)*q3(k1, k2, k3, mu, phi),nu3(k1,k2,k3,mu,phi))*(1.+beta*nu3(k1,k2,k3,mu,phi)**2.))*(kc1+k1*dk1/2.)*(kc2+k2*dk2/2.)*(kc3+k3*dk3/2.)*(dk1/2.)*(dk2/2.)*(dk3/2.))/apar**2./aperp**4.

		EPfunc = lambda k1,k2,k3,mu, phi: (b1*(2.*beta*nu1(mu)**2.*(1.+beta*nu1(mu)**2.)*Pres((kc1+k1*dk1/2.)*q1(mu),nu1(mu))+Pres((kc2+k2*dk2/2.)*q2(k1, k2, k3, mu, phi),nu2(k1,k2,k3,mu,phi))*(beta*nu2(k1,k2,k3,mu,phi)**2.*2.)*(1.+beta*nu2(k1,k2,k3,mu,phi)**2.)+ Pres((kc3+k3*dk3/2.)*q3(k1, k2, k3, mu, phi),nu3(k1,k2,k3,mu,phi))*(2.*beta*nu3(k1,k2,k3,mu,phi)**2.)*(1.+beta*nu3(k1,k2,k3,mu,phi)**2.))*(kc1+k1*dk1/2.)*(kc2+k2*dk2/2.)*(kc3+k3*dk3/2.)*(dk1/2.)*(dk2/2.)*(dk3/2.) + 2.*psh*(kc1+k1*dk1/2.)*(kc2+k2*dk2/2.)*(kc3+k3*dk3/2.)*(dk1/2.)*(dk2/2.)*(dk3/2.))/apar**2./aperp**4.
		...
		mat5 = EBfunc(*np.meshgrid(self.gauss_mu,self.gauss_mu,self.gauss_mu,self.gauss_mu2,self.gauss_mu2, sparse=False, indexing='ij'))
		...
		mat6 = EPfunc(*np.meshgrid(self.gauss_mu,self.gauss_mu,self.gauss_mu,self.gauss_mu2,self.gauss_mu2, sparse=False, indexing='ij'))
		...
		Pshotcov[3*self.ksize2 + j] = EBPshot[j]
		Bshotcov[3*self.ksize2 + j] = EBBshot[j]		

                mat7 = Bfunc4(*np.meshgrid(self.gauss_mu,self.gauss_mu,self.gauss_mu,self.gauss_mu2,self.gauss_mu2, sparse=False, indexing='ij'))
                c1cov[3*self.ksize2 + j] = np.matmul(np.matmul(np.matmul(np.matmul(np.matmul(mat7,self.gauss_w2)/2.,self.gauss_w2)/2.,self.gauss_w),self.gauss_w),self.gauss_w)/Nk1/Nk2/Nk3
